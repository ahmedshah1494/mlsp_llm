...
class EncodecEuclideanCodebook(nn.Module):
    ...
    def __init__(self, config: EncodecConfig):
        ...
        self.return_one_hot = config.differentiable_quantization

        embed = torch.zeros(config.codebook_size, config.codebook_dim)
        if config.differentiable_quantization:
    ...
    def quantize(self, hidden_states):
        ...
        if self.return_one_hot:
            embed_ind_soft = torch.softmax(dist, dim=-1)
            embed_ind_hard = torch.zeros_like(embed_ind_soft).scatter_(-1, embed_ind.unsqueeze(-1), 1.0)
            embed_ind_one_hot = embed_ind_soft + (embed_ind_hard - embed_ind_soft).detach()
            return embed_ind, embed_ind_one_hot
        return embed_ind
    ...
    def encode(self, hidden_states):
        ...
        if isinstance(embed_ind, tuple):
            embed_ind, embed_ind_one_hot = embed_ind
            embed_ind_one_hot = embed_ind_one_hot.view(*shape[:-1], -1)
        # post-process
        embed_ind = embed_ind.view(*shape[:-1])
        if 'embed_ind_one_hot' in locals():
            return embed_ind_one_hot
        return embed_ind

    def decode(self, embed_ind):
        if embed_ind.shape[-1] == self.codebook_size:
            quantize = torch.matmul(embed_ind, self.embed)
        else:
